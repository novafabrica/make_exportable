...
To export your model's data you can simply call the class method <em>to_export</em>
	...
	Customer.to_export("csv", :only => ['first_name', 'last_name'])
...
You can easily switch the format by substituting 'csv' for your desired format. 
 ...
 It also allows you to send finder options to the class. 
	...
	Customer.to_export("xml", :columns => ['first_name', 'last_name'], :conditions => {:last_name => "smith"}, :order => "DESC")
 ...
 * as - array of columns names and methods to be exported. 
 This defaults to all columns minus salt and hashed_password and password
...
<em>to_csv_export</em>, where "format" is the export format (as in "to_csv_report"), which calls **to\_export(format)**.

<em>create_format_for</em> (and <em>create_format_report</em>), where "format" is the export format (as in "create_csv_report"/"to_csv_export"), which calls <em>create_format_report</em>.
...
Author: Kevin Skoglund & Matthew Bergman, Nova Fabrica, Inc.</td>
