...
To export your model's data you can simply call the class method **to\_export**.
	...
	Customer.to_export("csv", ['first_name', 'last_name'])
...
You can easily switch the format by substituting 'csv' for your desired format. 
	...
	Customer.to_export("xml", ['first_name', 'last_name'])

**to\_export** also allows you to send conditions and order to the class. 

	Customer.to_export("xml", ['first_name', 'last_name']), :conditions => {:last_name => "smith"}, :order => "DESC"
...
MakeExportable doesn't just export database columns. It can also export data returned from methods.
	...
	Customer.to_export("csv", ['full_name'])
...
If you have a general conditions and an order you will be calling in multiple methods you can attach them to the **make\_exportable** method when including it into your class

	class Customer < ActiveRecord::Base
		make_exportable :conditions => {:active => true}, :order => 'last_name ASC'
	end
